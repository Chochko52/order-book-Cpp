...
## Основни възможности
- Поддържани типове поръчки: Good‑Until‑Cancel (GTC), Fill‑And‑Kill (FAK), Fill‑Or‑Kill (FOK), Good‑For‑Day (GFD) и Market (MKT).【F:main.cpp†L70-L116】【F:main.cpp†L1030-L1083】
- Странични поръчки (buy/sell) с агрегация на нива и автоматично съпоставяне (matching) при добавяне или модифициране.【F:main.cpp†L200-L287】【F:main.cpp†L1076-L1107】
- Цикъл за изчистване на GFD поръчки в края на деня и коректно управление на отменени/изпълнени заявки.【F:main.cpp†L280-L347】
- Публикуване на състоянието към `book_data.json`, което позволява директна визуализация в уеб интерфейса.【F:main.cpp†L230-L279】
- Проверка на интегритет и запис на статуса в `status.json` (използва се от панела „System Check“).【F:main.cpp†L348-L365】【F:app.py†L52-L89】
...
## Изисквания
- CMake ≥ 3.30 и компилатор с поддръжка на C++26.【F:CMakeLists.txt†L1-L6】
- Python 3.9+ с пакети `streamlit` и `pandas` за уеб интерфейса.【F:app.py†L1-L53】
...
3. Интерфейсът използва следните файлове в текущата папка:
   - `commands.txt` – UI записва команди, които C++ процесът изпълнява.【F:main.cpp†L1018-L1060】【F:app.py†L30-L89】
   - `book_data.json` – C++ публикува текущите нива и активни поръчки; UI ги визуализира.【F:main.cpp†L230-L279】【F:app.py†L9-L79】
   - `status.json` – резултат от системната проверка (панел „System Check“).【F:main.cpp†L348-L365】【F:app.py†L52-L89】
...
2. Изберете метод за въвеждане:
   - **Конзола**: изпълнете команда като `echo "add GTC buy 101.5 50" >> commands.txt` или използвайте интерактивно въвеждане в терминала, ако бинарникът приема вход от stdin.【F:main.cpp†L1024-L1072】
   - **Streamlit UI**: стартирайте `streamlit run app.py`, попълнете формата „Add Order“ и натиснете „Submit“. Интерфейсът записва еквивалентната команда в `commands.txt`.【F:app.py†L90-L177】
3. Изпратената команда се прочита от двигателя, който при възможност съпоставя ордъра с насрещни заявки и актуализира `book_data.json`. Визуализацията в UI-то се обновява автоматично при следващия рефреш.【F:main.cpp†L230-L279】【F:app.py†L9-L79】

Поръчките се изпълняват автоматично при възможност за съвпадение, като изпълнените сделки се отпечатват в конзолата. За целите на демо интерфейса модификациите очакват да знаете страната (buy/sell) на поръчката, което се проследява на база последно подадения тип.【F:main.cpp†L1030-L1166】
...
- Кодът използва мултитрединг за GFD цикъла; при добавяне на нови функции спазвайте заключванията (`ordersMutex_`).【F:main.cpp†L280-L347】
